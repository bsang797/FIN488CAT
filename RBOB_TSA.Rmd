...
opec_df <- tribble(~ticker, ~param,
  ...
  dplyr::mutate(value = case_when(param=='OPEC_production' ~ value*1000,
                                  param=='rbob_spot' ~ value*42*100+18000,
                                  TRUE ~ value))
...
opec_plt <- opec_df %>% filter(date>"1994-01-01") %>% filter(param %in% c('OPEC_production',
  ...
  scale_y_continuous(name = 'Thousand Barrels per month',
                     sec.axis = sec_axis(~./4200-4.2857, name = "RBOB Spot price/1000 barrels"),
                     labels = scales::comma)
