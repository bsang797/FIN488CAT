...
states_pop_long <- states_pop %>% 
  pivot_longer(cols = 2:ncol(states_pop), names_to = 'state', values_to='pop_change') %>% 
...
#plotting pop_change
pop_plt <- states_pop_long %>% summarise('pop_growth' = sum(pop_change)) %>% 
  ggplot(aes(x=date, y=pop_growth, fill=as.factor(padd))) + 
...
opec_df <- tribble(~ticker, ~param,
                   'STEO.COPR_OPEC.M', 'OPEC_production',
                   #consumption per PADD
                   'PET.MGFUPP12.M', 'ec_padd1',
                   'PET.MGFUPP22.M', 'midw_padd2',
                   'PET.MGFUPP32.M', 'gc_padd3',
                   'PET.MGFUPP42.M', 'rm_padd4',
                   'PET.MGFUPP52.M', 'wc_padd5',
                   # 'PET.MGFUPUS2.M', 'consumption',
...
opec_plt <- opec_df %>% filter(date>"1994-01-01") %>% filter(!param %in% c('OPEC_production',
                                                                           'rbob_spot')) %>% 
  ggplot(aes(x=date,y=value,color=param)) + 
  geom_line() +
  scale_y_continuous(name = 'Thousand Barrels per month',
                     sec.axis = sec_axis(~./500-1, name = "RBOB Spot price/1000 barrels"),
                     labels = scales::comma) %>% 
